...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: signInError } = await supabasePlatform.auth.signInWithPassword({
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabasePlatform.auth.resetPasswordForEmail(resetEmail, {
          ...
          {!showForgotPassword ? (
            <>
              ...
              <p className="mt-6 text-center" style={{ color: 'var(--text-secondary)' }}>
                ...
                <Link href="/app-signup" className="font-semibold" style={{ color: 'var(--brand-primary)' }}>
                  Start free trial
                </Link>
              </p>
            </>
